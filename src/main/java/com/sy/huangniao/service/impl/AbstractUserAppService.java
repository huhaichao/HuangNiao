...
public abstract class AbstractUserAppService implements UserAppService {
    ...
    public JSONObject payQuery(JSONObject jsonObject) {
         ...
         * 调用预下单接口
         */
        IWXPaychannelsService iwxPaychannelsService = hnContext.getIWXPaychannelsService(
            AppCodeEnum.valueOf(jsonObject.getString("appCode")));
    ...
    public JSONObject payOrder(JSONObject jsonObject) {
        ...
        if (ticketOrder2 == null) {
            log.info("支付订单-原订单不存在,userid={},id={},orderNo={}", ticketOrder.getUserId(), ticketOrder.getId(),
                ticketOrder.getOrderNo());
            throw new HNException(RespondMessageEnum.CANCLEORDERNOEXSIT);
        }

        if (!(ticketOrder2.getOrderStatus().equals(OrderStatusEnum.WAITPAY.getStatus()) || ticketOrder2.getOrderStatus()
            .equals(OrderStatusEnum.WAITPAYHANDLE.getStatus()))) {
            log.info("支付订单-订单状态异常,userid={},id={},orderNo={}", ticketOrder.getUserId(), ticketOrder.getId(),
                ticketOrder.getOrderNo());
        ...
        if (userDeposit2 == null) {
            log.info("支付订单-原充值订单不存在,重新发起预支付,userid={},id={},orderNo={}", ticketOrder.getUserId(), ticketOrder.getId(),
                ticketOrder.getOrderNo());
            return deposit(jsonObject);
        }

        if (!userDeposit2.getStatus().equals(WalletStatusEnum.DEPOSITING.getStatus())) {
            log.info("支付订单-充值状态异常,userid={},id={},orderNo={},depositNo={}", ticketOrder.getUserId(),
                ticketOrder.getId(),
        ...
        if (userDeposit2.getCreateDate().after(DateUtils.getDateInMinuteAgo(new Date(), -90))
            ...
            //如果订单在1办小时之内，继续支付
            log.info("支付订单-订单中1.5小时之内,重新发起支付,userid={},id={},orderNo={}", ticketOrder.getUserId(), ticketOrder.getId(),
                ticketOrder.getOrderNo());
            JSONObject result = new JSONObject();
            result.put("prepay_id", userDeposit2.getPrepayId());
            return handleUnifiedorder(result);
        } else {
            log.info("支付订单-订单已失效-重新发起预支付,userid={},id={},orderNo={}", ticketOrder.getUserId(), ticketOrder.getId(),
                ticketOrder.getOrderNo());
            ...
            if (iUserDepositDaoService.updateObject(userDeposit3, SqlTypeEnum.DEAFULT) != 1) {
                log.info("支付订单-关闭原来的订单失败,userid={},id={},orderNo={},depositNo={}", ticketOrder.getUserId(), ticketOrder
                        .getId(),
                    ticketOrder.getOrderNo(), userDeposit2.getDepositNo());
                throw new HNException(RespondMessageEnum.DEPOSITPAYREPEAT);
            }
